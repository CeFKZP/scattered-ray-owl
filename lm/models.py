...
class LanguageModel(nn.Module):
    def __init__(self,
        ...
        elif rnn_type == 'egrud_haste':
            self.rnns = [haste.EGRU(input_size=emb_dim if l == 0 else hidden_dim,
    ...
    def prune_embeddings(self, index):
        ...
        self.embeddings.weight = nn.Parameter(self.embeddings.weight[:, index]).to(device)
    ...
    def prune(self, fractions, hiddens, device):
        ...
        for i in range(self.nlayers):
            if isinstance(fractions, float):
                ...
                raise NotImplementedError(f"data type {type(fractions)} not implemented. Use float, tuple or list")
        ...
        print(f"Final model hidden size: {[rnn.hidden_size for rnn in self.rnns]}")
    ...
    def init_hidden(self, batch_size):
        ...
        elif self.rnn_type == 'egrud_haste':
    ...
    def forward(self, inputs, state):
        ...
        for l, rnn in enumerate(self.rnns):
            ...
            if self.rnn_type == 'egrud_haste':
                c, o, tr = final_states

                self.ar_loss += self.activity_regularization(hidden_raw=c, output_gates=o)
            ...
            if l == self.nlayers - 1 and self.rnn_type != 'egrud_haste':
    ...
    def set_backward_sparsity(self, l, cell_states):
        ...
        thresholds = self.rnns[l].module.thr if isinstance(self.rnns[l], WeightDrop) else self.rnns[l].thr
        thresholds = thresholds.to(cell_states.device)

        if self.rnn_type == 'egrud_haste':
            epsilon = self.rnns[l].pseudo_derivative_support
        else:
            raise NotImplementedError(f"RNN type {self.rnn_type} does not implement surrogate gradient")
