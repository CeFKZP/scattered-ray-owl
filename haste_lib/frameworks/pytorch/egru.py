...
from .gru import GRUScript, GRUFunction
...
class SpikeFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        dz_du = dampening_factor * torch.maximum(1 - pseudo_derivative_support * torch.abs(inp), torch.Tensor((0,)).to(grad_output.device))
...
def EGRUScript(
  ...
  for t in range(time_steps):
    ...
    event = SpikeFunction.apply(cur_h - thr, dampening_factor, pseudo_derivative_support)
    o.append(event)
...
class EGRUFunction(torch.autograd.Function):
  ...
  def forward(ctx, training, zoneout_prob, dampening_factor, pseudo_derivative_support, max_norm, *inputs):
    ...
    '''
    y, cache, h, o, trace = LIB.egru_forward(training, zoneout_prob, *inputs)
    ctx.save_for_backward(inputs[0], *inputs[2:], dampening_factor, pseudo_derivative_support, max_norm, y, h, cache)
  ...
  def backward(ctx, grad_y, grad_h, grad_o, grad_trace):
    ...
    if not ctx.training:
      raise RuntimeError('EGRU backward can only be called in training mode')
    ...
    *grads, grad_scaling_factor = LIB.egru_backward(*saved, grad_y.contiguous(), grad_h.contiguous(), grad_o.contiguous(), grad_trace.contiguous())
    grads = grads[:-2]
...
class EGRU(BaseRNN):
  ...
  def __init__(self,
    ...
    self.dampening_factor = nn.Parameter(torch.Tensor([dampening_factor]), requires_grad=False)
    self.pseudo_derivative_support = nn.Parameter(torch.Tensor([pseudo_derivative_support]), requires_grad=False)
  ...
  def grad_clip_norm(self, enable=True, norm=2.0):
    ...
    self._max_norm = nn.Parameter(torch.Tensor([norm if enable else -1.0]), requires_grad=False)
  ...
  def forward(self, input, state=None, lengths=None):
    ...
    y, h, o, trace = self._impl(input, h0[0], thr, self._get_zoneout_mask(input))
