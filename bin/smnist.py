...
def sequential_MNIST(batch_size, cuda, data_path, permute=False):
    ...
    full_train_set = datasets.MNIST(data_path, train=True, download=True, transform=transforms.Compose(transforms_list))
    ...
    train_set, val_set = torch.utils.data.random_split(full_train_set, [train_set_size, valid_set_size], generator=seed)
    ...
    test_loader = torch.utils.data.DataLoader(
        datasets.MNIST(data_path, train=False, transform=transforms.Compose(transforms_list)),
...
def train_batch(model, loss_function, optimizer, ext_inputs, targets, train_params, c, device):
    ...
    if c.rnn_type == RNNType.EGRUD_HASTE:
...
def get_rnn(input_size, c):
    if c.rnn_type == RNNType.LSTM:
        ...
        if c.custom_lstm_init:
            ...
            for name, param in rnn.named_parameters():
                if 'bias' in name:
                    nn.init.ones_(param[c.n_units * c.unit_size:2 * c.n_units * c.unit_size])
                elif 'weight' in name:
                    nn.init.xavier_uniform_(param[:c.n_units * c.unit_size])
                    nn.init.xavier_uniform_(param[c.n_units * c.unit_size:2 * c.n_units * c.unit_size])
                    nn.init.xavier_uniform_(param[2 * c.n_units * c.unit_size:3 * c.n_units * c.unit_size])
                    nn.init.xavier_uniform_(param[3 * c.n_units * c.unit_size:4 * c.n_units * c.unit_size])
    elif c.rnn_type == RNNType.GRU:
        assert c.unit_size == 1
        rnn = haste.GRU(input_size, c.n_units * c.unit_size, dropout=c.dropout_connect, zoneout=c.zoneout,
                        batch_first=True)
    elif c.rnn_type == RNNType.EGRUD_HASTE:
        rnn = haste.EGRU(input_size, c.n_units, dropout=c.dropout_connect, zoneout=c.zoneout, batch_first=True,
    ...
    print(f"{c.rnn_type} parameters: ", list(map(lambda x: x[0], rnn.named_parameters())))
    return rnn
...
def main(c, pt):
    ...
    for epoch in range(resume_epoch, c.n_training_epochs):
        torch.cuda.empty_cache()
        if pt:
            torch.save(model.state_dict(), os.path.join(pt.results_path, f'models/egurc-{epoch}.pt'))
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--voltage-regularization-constant', type=float, default=1.)
    argparser.add_argument('--voltage-regularization-target', type=float, default=-0.9)
    ...
    def get_config():
        ...
        if rnn_type in [RNNType.LSTM, RNNType.GRU, RNNType.EGRUD_HASTE]:
            n_units = args.units
        else:
